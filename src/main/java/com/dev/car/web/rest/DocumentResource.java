...
import com.dev.car.domain.Document;
import com.dev.car.repository.DocumentRepository;
...
/**
 * REST controller for managing {@link com.dev.car.domain.Document}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class DocumentResource {
    ...
    public List<Document> getAllDocuments(@RequestParam(required = false) String filter) {
        if ("car-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where car is null");
            return StreamSupport
                .stream(documentRepository.findAll().spliterator(), false)
                .filter(document -> document.getCar() == null)
